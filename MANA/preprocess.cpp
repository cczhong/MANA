...
static unsigned int DEFAULT_MIN_LENGTH_AA = 15;
...
"      -a, --amino-acid                 input sequences are amino acids instead of DNA\n"
...
namespace opt
{
    ...
    static bool processAminoAcid = false;
...
static const char* shortopts = "o:q:m:h:p:r:c:s:f:vai";
...
{
    Timer* pTimer = new Timer("MANA preprocess");
    ...
    if(opt::peMode == 0)
    {
        ...
        while(optind < argc)
        {
            ...
            // Note: the reads are loaded in here
            // SeqReader.cpp locates at ./Util
            SeqReader reader(filename, SRF_NO_VALIDATION);
            // SeqRecord define in ./Util/Util.h
            SeqRecord record;
            ...
            while(reader.get(record))
            {
                ...
                bool passed = processRead(record, opt::processAminoAcid);
                // DEBUG
    ...
    else
    {
        ...
        while(optind < argc)
        {
            ...
            while(pReader1->get(record1) && pReader2->get(record2))
            {
                ...
                bool passed1 = processRead(record1, opt::processAminoAcid);
                bool passed2 = processRead(record2, opt::processAminoAcid);
...
bool processRead(SeqRecord& record, bool isAminoAcid)
{
/* DEBUG
    if(isAminoAcid)
    {
      std::cout << "processing amino acids\n";
      exit(0);
    }
*/
    // let's remove the adapter if the user has requested so
    // before doing any filtering
    // we do not perform adapter trimming if working on amino acid reads
    if(!isAminoAcid && !opt::adapterF.empty())
    {
    ...
    if(!opt::bDiscardAmbiguous)
    {
        ...
        // Convert non-standard nucleotide to a random nucleotide
        // or convert non-standard amino acid to a random amino acid
           
        for(size_t i = 0; i < seqStr.size(); ++i)
        {
            // Convert '.' to 'N'
            if(seqStr[i] == '.')
                seqStr[i] = isAminoAcid ? 'X' : 'N';
            // IUPAC is defined in ./Utils/Alphabet.cpp
            if( (!isAminoAcid && !IUPAC::isAmbiguous(seqStr[i])) ||
                (isAminoAcid && !IUPAC_AA::isAmbiguous(seqStr[i]))
            )
                continue;

            // Get the string of possible bases for this ambiguity code
            std::string possibles = isAminoAcid ? IUPAC_AA::getPossibleSymbols(seqStr[i]) : IUPAC::getPossibleSymbols(seqStr[i]);
    ...
    // Ensure sequence is entirely ACGT (for DNA) or ARNDCQEGHILKMFPSTWYV (for amino-acid)
    size_t pos = isAminoAcid ? seqStr.find_first_not_of("ARNDCQEGHILKMFPSTWYV") : seqStr.find_first_not_of("ACGT");
    ...
    // we do not expect quality input with amino acid reads
    if(!isAminoAcid && !qualStr.empty())
    {
    ...
    // Quality trim
    if(!isAminoAcid && opt::qualityTrim > 0 && !qualStr.empty())
        softClip(opt::qualityTrim, seqStr, qualStr);

    // Quality filter
    if(!isAminoAcid && opt::qualityFilter >= 0 && !qualStr.empty())
    {
    ...
    // Dust filter
    // we do not filter amino acid sequences
    if(!isAminoAcid && opt::bDustFilter)
    {
        // DustScore calculated in ./Util/Util.cpp
    ...
    // Primer screen; no available for amino acids
    if(!isAminoAcid && !opt::bDisablePrimerCheck)
    {
...
void parsePreprocessOptions(int argc, char** argv)
{
    ...
    for (char c; (c = getopt_long(argc, argv, shortopts, longopts, NULL)) != -1;)
    {
        ...
        switch (c)
        {
            ...
            case 'm': arg >> opt::minLength; opt::isLengthSet = true; break;
            ...
            case 'a': opt::processAminoAcid = true; if(!opt::isLengthSet) opt::minLength = DEFAULT_MIN_LENGTH_AA; break;
            case '?': die = true; break;
    ...
    if( (!opt::processAminoAcid && opt::minLength < DEFAULT_MIN_LENGTH) || (opt::processAminoAcid && opt::minLength < DEFAULT_MIN_LENGTH_AA) )
    {
        std::cerr << SUBPROGRAM ": WARNING - it is suggested that the min read length is: \n";
        std::cerr << SUBPROGRAM ": " << DEFAULT_MIN_LENGTH << "for DNA reads;\n";
        std::cerr << SUBPROGRAM ": " << DEFAULT_MIN_LENGTH_AA << "for short-peptide reads.\n";
