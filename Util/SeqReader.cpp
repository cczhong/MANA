...
bool SeqReader::get(SeqRecord& sr)
{
    ...
    if(validRecord)
    {
        ...
        // If the DNA validation flag is set, ensure that there aren't any non-ACGT bases
        if(m_flags & SRF_VALIDATE_DNA)
        {
        ...
        // If the Protein validation flag is set, ensure that there aren't any amino acid residue
        if(m_flags & SRF_VALIDATE_PROT)
        {
            if(seq.find_first_not_of("ARNDCQEGHILKMFPSTWYV") != std::string::npos)
            {
                std::cerr << "Error: read " << sr.id << " contains non-amino acid characters.\n";
                std::cerr << "Please run sga preprocess on the data first.\n";
                exit(EXIT_FAILURE);
            }
        }
